...
import {registerDevTool} from './devTools.js';

// re-export the developer-mode public API (flag gate, tool-registration API,
// overlay toggle signal, the typed dev-log, and the opt-in key-trigger helper)
export {createTripleKeyDetector, isDevModeEnabled} from './devMode.js';
export {
  closeDevOverlay, getDevTools, openDevOverlay, registerDevTool,
  toggleDevOverlay, unregisterDevTool
} from './devTools.js';
export {clearDevLog, devLog, getDevLogEntries} from './devLog.js';
...
export async function bootstrap() {
  ...
  // when dev mode is enabled, lazy-load the dev-mode overlay shell as its own
  // chunk; the import is only referenced behind the flag, so production builds
  // (flag off) never fetch or execute it
  const _devOverlay = isDevModeEnabled() ? defineAsyncComponent(() => import(
    /* webpackChunkName: "bedrock-vue-devmode" */
    '../components/DevModeOverlay.vue')) : null;

  // auto-register the built-in typed dev-log tool so `devLog()` works with no
  // setup; lazily loaded into the same dev-mode chunk
  if(_devOverlay) {
    registerDevTool({
      id: 'dev-log',
      label: 'Log',
  ...
  const app = _state.rootApp = createApp({
    ...
    // render the root Vue; when dev mode is on, render the overlay shell as a
    // sibling so it is reachable on every page without app layout changes
    render: () => _devOverlay ?
      [h(_state.rootVue), h(_devOverlay)] : h(_state.rootVue)
  });
